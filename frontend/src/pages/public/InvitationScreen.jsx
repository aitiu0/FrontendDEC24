...
const InvitationScreen = () => {
	const dispatch = useAppDispatch()
  	const { isLoaded, eventName, eventDate, eventTime, guessName, guessLastName, guessResponse, guessAccept, guessAllergies, guessPlusOne } = useAppSelector((state) => state.invitation)
	...
	const navigate = useNavigate();
  	...
  	useEffect(() => {
  		axios.get(`http://localhost:5001/api/users/${idInvitado}`)
	      .then(response => {
	        dispatch(updateEventDetails({ guessName: response.data.name }));
	        dispatch(updateEventDetails({ guessLastName: response.data.lastName }));
	      })
	      .catch(error => {
  	...
  	const handleReject = () => {
	    ...
	    axios.post(`http://localhost:5001/api/users/invite-response/${idInvitado}`, {
	      accept: guessAccept,
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="h-max bg-white p-8 flex flex-col justify-between">
          ...
          >
            <h2 className="text-3xl font-serif text-gray-800 mb-4">{eventName}</h2>
            <p className="text-gray-600">Le invitamos cordialmente a experimentar la elegancia y el lujo en nuestro exclusivo evento Open House.</p>
            <motion.img 
	            src={casaLogo}
	            alt="Luxury Property" 
          ...
          >
            <div>
           	  <p className="text-gray-900 font-bold">{guessName} {guessLastName}</p>
              <p className="text-gray-800 font-semibold">{eventDate}</p>
              <p className="text-gray-600">{eventTime}</p>
              <p className="text-black">Código de vestimenta: elegante y sofisticado, con traje o vestido formal apropiado para un evento de lujo</p>
            </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96 border border-gray-300">
            <h3 className="text-xl font-semibold mb-4">Confirmar Asistencia</h3>
            <div>
              <label className="block text-sm font-semibold mb-2">Alergias</label>
              <textarea 
                name="allergies" 
            ...
            <div className="mt-4">
              <label className="block text-sm font-semibold mb-2">¿Asistirá con acompañante?</label>
              <select 
                name="plusOne" 
